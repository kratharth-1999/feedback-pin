...
const API_BASE_URL = "https://triumphant-wholeness-production-8438.up.railway.app";
...
export const apiService = {
  ...
  getPinsByUrl: async (url: string, emailId: string): Promise<PinType[]> => {
    try {
      const response = await fetch(`${API_BASE_URL}/pins?url=${encodeURIComponent(url)}&emailId=${encodeURIComponent(emailId)}`);
      const result = await response.json();
      
      if (!response.ok) {
        throw new Error(result.message || "Failed to fetch pins for URL");
      }
  ...
  savePin: async (pin: PinType): Promise<void> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || "Failed to create/update pin");
      }
    } catch (error) {
  ...
  deletePin: async (id: string): Promise<void> => {
    try {
      const response = await fetch(`${API_BASE_URL}/pin?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
  ...
  deletePinsByUrl: async (url: string, emailId: string): Promise<void> => {
    try {
      const response = await fetch(`${API_BASE_URL}/pin?url=${encodeURIComponent(url)}&emailId=${encodeURIComponent(emailId)}`, {
        method: "DELETE",
      });
      ...
      if (!response.ok) {
        throw new Error(result.message || "Failed to delete pins for URL");
      }
    } catch (error) {
