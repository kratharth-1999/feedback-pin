...
const Overlay: React.FC<OverlayProps> = ({ isActive, showPins }) => {
  ...
  const handlePinUpdate = useCallback((updatedPin: PinType) => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (!target.closest(".feedback-form") && !target.closest(".pin-details") && !target.closest(".pin")) {
        setShowFeedbackForm(false);
        setShowPinDetails(false);
      }
    };
  ...
  return (
    <div className="overlay" onClick={handleOverlayClick}>
      {showPins && pins.filter(pin => pin.path === window.location.href).map(pin => (
