...
export function usePins(emailId: string) {
  ...
  useEffect(() => {
    const loadPins = async () => {
      ...
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : "Error loading pins";
  ...
  const addPin = useCallback(async (pin: PinType) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Error adding pin";
      toast.error(errorMessage);
  ...
  const removePin = useCallback(async (pinId: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Error removing pin";
      toast.error(errorMessage);
  ...
  const updatePin = useCallback(async (updatedPin: PinType) => {
    ...
    try {
      ...
      await apiService.savePin(pinWithEmail);
      setPins(prevPins => 
        prevPins.map(pin => pin.id === updatedPin.id ? pinWithEmail : pin)
      );
      toast.success("Pin updated successfully");
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Error updating pin";
      toast.error(errorMessage);
  ...
  const removeAllPinsByPath = useCallback(async (path: string) => {
    setIsLoading(true);
    try {
      await apiService.deletePinsByUrl(path, emailId);
      setPins(prevPins => prevPins.filter(pin => pin.path !== path));
      toast.success("All pins removed successfully");
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Error removing pins";
      toast.error(errorMessage);
